...
export default {
  ...
  watch(
    ...
    const userFile = ref({ data: null, mime_type: null, preview: null });
     const messages = ref([
  {
    text: "親愛的貴賓您好👋  <br /> 我是Gururu智能客服~ <br /> 請問今天有什麼需要幫忙的呢?",
    role: "bot-message",
    ...
    // --- 使用者登入資訊 ---
    const userInfo = ref({ isLoggedIn: false, userName: "", roleId: null, userId: null });

    const fetchUserSession = async () => {
      try {
        const res = await axios.get("/api/currentUser", { withCredentials: true });
        const data = res.data;
        if (data.user) userInfo.value = { ...data.user, isLoggedIn: true };
        else userInfo.value.isLoggedIn = false;
      } catch (err) {
        console.error(err);
  ...
  return `${qa.A || ""}${targetUrl ? `<br><a href="${targetUrl}" class="qa-link" target="_blank">👉🏻 點我前往</a>` : ""}`;
    ...
    // --- OpenAI ---
    const OPENAI_API_KEY = "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";
    ...
    return {
      ...
      onSelectEmoji,
      isListening,
      startListening, // 回傳給 template 用按鈕啟動語音
    };
  },
};
